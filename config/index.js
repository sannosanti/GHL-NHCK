...
const constants = {
  ...
  // Luisa's (NHC adultos) triage categories — used when a conversation started
  // by Carolina gets handed off mid-thread because the patient turns out to
  // be 18+. Keep in sync with GHL-NHC-temp/config/index.js by hand.
  TRIAJE_P1_ADULTOS: ['Ansiedad constante', 'Ánimo bajo/depresión', 'Problemas de sueño', 'Falta de concentración', 'Desregulación emocional', 'Agotamiento/burnout', 'Otro'],
  TRIAJE_P2_ADULTOS: ['Menos de 1 mes', '1–3 meses', '3–6 meses', 'Más de 6 meses', 'Más de 1 año'],
  TRIAJE_P3_ADULTOS: ['Terapia', 'Medicación', 'Varias sin resultado', 'Nada aún', 'Otro'],
};
...
Si quien pregunta es mayor de edad → avisale con calidez que la vas a conectar con Luisa, quien maneja los casos de adultos en NHC, y que ella misma sigue ayudándolo por este mismo chat. NUNCA digas que "un asesor lo va a contactar" ni te despidas — la conversación continúa de inmediato → [NHC_ADULTOS]
...
// ─── BASE DE CONOCIMIENTO NHC (ADULTOS) — para cuando Carolina deriva a Luisa ─
// Copiado literal de GHL-NHC-temp/config/index.js. Mantener en sync a mano.
const CONOCIMIENTO_NHC_ADULTOS = `
CONOCIMIENTO BASE — Luisa debe usar esto para responder con precisión:
...
## RANGO DE EDAD
- Atendemos exclusivamente a personas mayores de 18 años, sin tope superior
- Si quien escribe es menor de edad, o pregunta por la evaluación para un hijo/a menor de edad → explicá que esta línea es solo para adultos, que existe una línea para niños, y escalá para que un asesor conecte con la línea correspondiente → [ESCALAR]

## REQUISITOS PARA EL NEUROMAPEO
El procedimiento puede no realizarse si el paciente presenta:
1. Dificultad severa para seguir instrucciones básicas (no puede permanecer sentado, abrir/cerrar ojos cuando se le pide)
2. Movimiento excesivo e incontrolable durante la toma
3. Ausencia de control inhibitorio (no puede mantener estados específicos por algunos minutos)
4. Hipersensibilidad sensorial importante (rechazo al contacto en cabeza/cuello/hombros o intolerancia al gel)
5. Ansiedad o rechazo severo al procedimiento
6. Cualquier caso de autismo (TEA), en cualquier nivel: evaluar individualmente
El criterio principal es si el paciente puede tolerar el procedimiento y generar un registro de calidad clínica.

## AUTISMO (TEA) Y ENFERMEDADES
- Cualquier mención de autismo, TEA o Asperger, en cualquier nivel o sin especificar → [ESCALAR] siempre para evaluación individual.
- TDAH, déficit de atención, ansiedad, bajo rendimiento (laboral o académico), problemas de conducta, desregulación emocional → NO escalar. Son exactamente los casos que tratamos. Continuá con el flujo normal.
- Epilepsia activa no controlada, hipersensibilidad sensorial severa, imposibilidad de seguir instrucciones básicas → [ESCALAR] porque pueden impedir físicamente el procedimiento.
- Para otras condiciones: el criterio no es el diagnóstico sino la funcionalidad. Si el paciente puede seguir instrucciones básicas y tolerar el contacto, puede proceder. En caso de duda, preguntá sobre eso antes de escalar.
...
## CUANDO PIDEN DIAGNÓSTICO FORMAL
Si el paciente menciona que su EPS, su trabajo o una institución pide un diagnóstico formal → contemplar Evaluación Neuropsicológica → escalar al asesor

## EVALUACIÓN NEUROPSICOLÓGICA (diferente al neuromapeo)
- Evalúa perfil cognitivo: memoria, conducta, aprendizaje, emociones
- Genera diagnóstico formal reconocido por instituciones y entidades
- Luisa NO puede agendar esto — debe escalar al asesor
...
## QUÉ TRATAMOS
Ansiedad, estrés, TDAH/déficit de atención, memoria y concentración, depresión, insomnio, desregulación emocional, conducta impulsiva, bajo rendimiento (laboral o académico) — con neurotecnologías, sin medicamentos ni efectos secundarios.
...
## NEUROMAPEO PREVENTIVO
Si el cliente indica que NO tiene dificultades aparentes, NO descartarlo. Responder:
"El Neuromapeo también puede realizarse de manera preventiva 🧠 Muchas personas lo usan para conocer cómo está funcionando su cerebro, identificar fortalezas y áreas de oportunidad, y tener una línea base para potenciar su desarrollo."
...
- NUNCA mostrar tags internos como [ESCALAR] al usuario
- NUNCA hacer promesas de efectividad del tratamiento — la efectividad no depende 100% del centro
- NUNCA generar presión ni urgencia en la toma de decisión — las personas llegan en un estado emocional sensible; acompañar, no presionar
- NUNCA escribir "[Pendiente confirmar]" ni ningún placeholder — si el dato no está en el conocimiento base, decilo con naturalidad: "eso lo confirmaría el equipo el día de la cita"
- Si no tenés respuesta clara → respondé lo más útil que puedas con el conocimiento base. Solo escalá si el caso requiere genuinamente atención personalizada que Luisa no puede dar.
- Cuando se escala: NO cerrar la conversación por inactividad
`;
...
function mapearSintomaAdulto(s) {
  s = (s || '').toLowerCase().trim();
  if (s.includes('ansiedad')) return 'Ansiedad';
  if (s.includes('ánimo') || s.includes('animo') || s.includes('depres')) return 'Depresión';
  if (s.includes('sueño') || s.includes('sueno') || s.includes('insomnio')) return 'Insomnio';
  if (s.includes('concentra') || s.includes('atenc') || s.includes('deficit') || s.includes('déficit')) return 'Déficit de atención adultos';
  if (s.includes('burnout') || s.includes('agotamiento') || s.includes('estrés') || s.includes('estres')) return 'Estrés';
